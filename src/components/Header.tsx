...
const Header: React.FC = () => {
  ...
  const userMenu = (
    <Menu>
      <Menu.Item key="1" icon={<SettingOutlined />} onClick={() => {
        // 跳转到家庭管理区域
        const familyManagerElement = document.querySelector('.ant-card:has(.family-manager-title)');
  ...
  return (
    <AntHeader style={{ 
      ...
      <Menu mode="horizontal" selectedKeys={[location.pathname]} style={{ background: 'transparent', borderBottom: 0 }}>
        <Menu.Item key="/" icon={<FileTextOutlined />}>
          <Link to="/" style={{ color: '#666' }}>账单管理</Link>
        </Menu.Item>
      </Menu>
